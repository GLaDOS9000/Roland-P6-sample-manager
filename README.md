...
If you prefer to run from source (e.g. to modify the code yourself):
...
the interactive Python console (the `>>>` prompt).

#### 2.2.2 Install uv (recommended) or pip

**With uv** (fastest, handles the venv and all deps automatically):

```
pip install uv
```

**Without uv** - use pip directly (see below).
...
> The venv must be reactivated (`source p6env/bin/activate`) in every new
> terminal session when using the plain-venv approach.
...
src/pyp6/
  ...
  ui/
    ...
    dialogs.py                 themed message/input dialogs
    waveform.py                waveform canvas drawing helpers
    nav_mixin.py               shared folder-navigation mixin
    dialogs/                   file, preset, audio, pad, bank, synth, settings dialogs
  model/
    sample_slot.py             SampleSlot (one pad)
...
## 8. Building Your Own Standalone Windows Executable (Optional)
