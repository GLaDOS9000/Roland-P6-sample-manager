...
Startup profiling is enabled automatically at DEBUG log level.
Enable with:  pyp6 --debug   or   pyp6 --log-level DEBUG
"""
...
def _log_timing(label):
    ...
    logger.debug(f"[startup] {label}: {_time.perf_counter() - _t_start:6.3f}s elapsed")
...
# The logo, embedded so the app needs no companion file.
PYP6_LOGO_PNG = (
...
def _verify_ui_font(root):
    ...
    try:
        ...
        if UI_FAMILY not in available:
            logger.warning(
                f"UI font '{UI_FAMILY}' is not installed — "
                "falling back (this can noticeably slow down window drawing)."
            )
            for candidate in (
                "DejaVu Sans",
                "Liberation Sans",
                ...
                if candidate in available:
                    logger.info(f"Font suggestion: '{candidate}' is available on this system.")
                    break
    except Exception:
        pass
...
def _parse_args():
    parser = argparse.ArgumentParser(
        prog="pyp6",
        description="Roland AIRA P-6 Sample Manager",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable DEBUG level logging (equivalent to --log-level DEBUG).",
    )
    parser.add_argument(
        "--log-level",
        default=None,
...
def main():
    ...
    # Honour PYP6_DEBUG env var for backward compat
    env_debug = bool(_os.environ.get("PYP6_DEBUG"))
    debug = args.debug or env_debug
    level = "DEBUG" if debug else (args.log_level or "INFO")

    from pyp6.log import configure_logging
    ...
    if DND_AVAILABLE:
        logger.debug("tkinterdnd2 available — drag & drop enabled.")
    else:
        logger.debug(f"tkinterdnd2 NOT available: {DND_IMPORT_ERROR}")
    ...
    if DND_AVAILABLE:
        try:
            ...
            logger.warning(
                f"tkinterdnd2 failed to initialize ({e}), continuing without drag & drop"
            )
